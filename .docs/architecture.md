...
`evaluate_plasma_grids()` is the common data path for frontend Plotly fields and offline Matplotlib montages. `resolution` is the poloidal sample count and radial resolution is half as large; both renderers therefore evaluate identical plasma-aligned coordinates and display-ready flux/log-residual values. `evaluate_plasma_kpis()` separately evaluates `|R_GS|` statistics on a deterministic, area-uniform sqrt-rho Sobol sample (16,384 points/config by default), including whole-plasma/core distribution summaries, edge p95, and boundary leakage. The same KPI mapping is shown in both frontend benchmark views and `plot_plasma_grid_montage()` output.
