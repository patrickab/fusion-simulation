...
`evaluate_plasma_grids()` is the common data path for frontend Plotly fields and offline Matplotlib montages. `resolution` is the poloidal sample count and radial resolution is half as large; both renderers therefore evaluate identical plasma-aligned coordinates and display-ready flux/log-residual values. `evaluate_residual_samples()` is the batched, jitted KPI core: per-point `|R_GS|` of shape (n_configs, sample_size) on a deterministic, area-uniform sqrt-rho Sobol sample, vmapped over configs with a chunked inner point map and ψ computed once per point (reused for the axis estimate). `evaluate_plasma_kpis()` wraps it into whole-plasma/core distribution summaries, edge p95, and boundary leakage. Budgets are calibrated (`docs/evaluation/kpi-accuracy-benchmark.md`): benchmark/HPO-ranking evals use 4,096 points × 100 configs (`DEFAULT_KPI_SAMPLE_SIZE`); training-time validation tracks the same median-`|R_GS|` KPI at 1,024 points (`TRACKING_KPI_SAMPLE_SIZE`, `val_kpi_median` in training.csv, also the HPO pruning signal — the old composite-loss `val_loss` survives only in legacy CSVs). The same KPI mapping is shown in both frontend benchmark views and `plot_plasma_grid_montage()` output.
